...
import {
  ...
  ChevronDown,
  Loader2,
  Tag as TagIcon, // Rename Tag icon to TagIcon
} from "lucide-react";
import {
  Tooltip,
  TooltipContent,
...
import type {
  ...
  OutfitItem,
  Tag,
} from "@/app/models/types"; // Import OutfitItem and Tag
...
interface PublicOutfit extends Omit<Outfit, "user"> {
  // Omit the original 'user' field
  ...
  hasUpvoted: boolean;
  hasSaved: boolean;
  tags: Tag[]; // Use the proper Tag type
}
  ...
  return (
    <div className="mt-12 sm:mt-0 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-4">
      {Array.from({ length: 10 }).map(
        (
          _,
          index // Show 10 skeletons initially
        ) => (
          <div
            key={index}
            className="rounded-xl border border-border overflow-hidden animate-pulse bg-neutral-800/50"
          >
...
export default function DiscoverPage() {
  ...
  const [processingUpvote, setProcessingUpvote] = useState<Set<string>>(
    new Set()
  );
  ...
  const fetchOutfits = useCallback(
    async (query: string = "", isLoadMore = false) => {
      ...
      try {
        const url = `/api/outfits/public?${
          query ? `query=${encodeURIComponent(query)}` : ""
        }${isLoadMore && nextCursor ? `&cursor=${nextCursor}` : ""}`;
        ...
        setOutfits((prev) =>
          isLoadMore ? [...prev, ...(data.outfits || [])] : data.outfits || []
        );
  ...
  const handleToggleUpvote = async (outfitId: string, e: React.MouseEvent) => {
    ...
    if (!session) {
      toast({
        ...
        description: "Please sign in to upvote outfits",
      });
      return;
    }
    if (processingUpvote.has(outfitId)) return;
    ...
    const optimisticCount = optimisticUpvoted
      ? originalOutfit.upvoteCount + 1
      : Math.max(0, originalOutfit.upvoteCount - 1);
    ...
    try {
      ...
      setOutfits((prev) =>
        prev.map((o) =>
          o.id === outfitId
            ? { ...o, hasUpvoted: result.hasUpvoted, upvoteCount: result.upvoteCount }
            : o
        )
      );
    } catch (err) {
  ...
  const handleToggleSave = async (outfitId: string, e: React.MouseEvent) => {
    ...
    setOutfits((prev) =>
      prev.map((o) =>
        o.id === outfitId ? { ...o, hasSaved: optimisticSaved } : o
      )
    );
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className="relative rounded-full border border-border bg-background px-2 sm:px-3 py-1.5 text-sm flex items-center w-[220px] sm:w-[250px] focus-within:ring-2 focus-within:ring-primary focus-within:border-primary transition-all">
                ...
                />
                {isSearching && (
                  <Loader2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground animate-spin ml-1.5 sm:ml-2" />
                )}
              </div>
            </div>
          </div>
        ...
        {loading && !isSearching && outfits.length === 0 ? (
          ...
          <div className="text-center py-16">
            <p className="text-muted-foreground mb-4 text-lg">
              {searchQuery
                ? "No outfits found for your search."
                : "No fits to discover... yet!"}
            </p>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <p className="text-xs font-medium">
                      <PriceDisplay
                        ...
                        showOriginal={outfit.costCurrency !== currency}
                        showTooltip={true}
                      />
                    </p>
                    ...
                    <div className="flex items-center gap-1 sm:gap-2">
                      <button
                        ...
                        aria-label={
                          outfit.hasSaved ? "Unsave outfit" : "Save outfit"
                        }
                      >
